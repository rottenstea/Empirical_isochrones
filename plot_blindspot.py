...
for axis in ax:
    xlim = axis.get_xlim()
    # axis.hlines(y=0, xmin=xlim[0], xmax=xlim[1], ls="--", color="darkslategray", alpha = 0.75)
    axis.hlines(y=10, xmin=xlim[0], xmax=xlim[1], ls="-.", color="darkslategray", alpha=0.75, lw=1)

fig_2D.show()

if save_plot:
    fig_2D.savefig(output_path + "Blindspot_vs_empirical.pdf", dpi=600)
...
fig_3, ax = plt.subplots(1, 3, figsize=(7.24551, 4.), sharey="row")
...
ax[0].set_ylabel(r"$\mathrm{M}_{\mathrm{G}}$", labelpad=1)
...
fig_3.show()
if save_plot:
    fig_3.savefig(output_path + "Blindspot_Gaia_limits.pdf", dpi=600)
