...
#Pleiades_df = Pleiades_df[Pleiades_df["BP-RP"] != 0]
...
for ax in axes:
    ax.scatter(OC.density_x, OC.density_y, **OC.kwargs_CMD, label="data")
...
#  ax.plot(CG["BP-RP"][start:stop], CG["Gmag"][start:stop], color="#e6ab02", label="78 Myr")
...
ax1.set_xlabel(r"$\mathrm{G}_{\mathrm{BP}} - \mathrm{G}_{\mathrm{RP}}$", labelpad=1, x=1.)
