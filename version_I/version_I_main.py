...
for cluster in CII_clusters[:]:
    OC = star_cluster(cluster, CII_df, CMD_parameters=["Gmag","BP-RP"])
    ...
    if not preprocess:
        ...
        try:
            params = SVR_read_from_file(hypers + "/PCA/" + "HP_{}.txt".format(OC.name))
        except FileNotFoundError:
            pca = PCA(n_components=2)
            pca_arr = pca.fit_transform(OC.CMD)
            tuned_params = SVR_Hyperparameter_tuning(pca_arr, output_file=hypers + "HP_{}.txt".format(OC.name))
            params = SVR_read_from_file(hypers + "/PCA/" + "HP_{}.txt".format(OC.name))
    ...
    if not preprocess:
        ...
        ax.plot(median[:, 0], median[:, 1], label="Isochrone", color="red")
