...
CIII_cols = ["cluster_name", "Plx", "e_Plx", "Gmag", "e_Gmag", "BPmag", "e_BPmag", "RPmag", "e_RPmag", "BP-RP",
             ...
             "ruwe", "fidelity_v2", "stability"]

CIII_names = ["Cluster_id", "plx", "e_plx", "Gmag", "e_Gmag", "BPmag", "e_BPmag", "RPmag", "e_RPmag", "BP-RP",
              "BP-G", "G-RP",
              "age_lts", "age_tdist", "ruwe", "fidelity", "stability"]

q_filter = {"parameter": ["ruwe", "plx", "fidelity", "stability"], "limit": ["upper", "lower", "lower", "lower"], "value": [1.4, 0, 0.5, 6]}


CIII_clusters, CIII_df = create_df(CIII_raw, CIII_cols, CIII_names, q_filter)


for cluster in CIII_clusters[:10]:
    OC = star_cluster(cluster, CIII_df, CMD_parameters=["Gmag","BP-RP"])
    if OC.Nstars >= 100:
        ...
        if not preprocess:
            try:
                ...
                tuned_params = SVR_Hyperparameter_tuning(hypers)
                params = SVR_read_from_file(hypers + "HP_{}.txt".format(OC.name))
            ...
            try:
                ...
                tuned_params = OC.SVR_Hyperparameter_tuning(file_path=hypers + "HP_{}.txt".format(OC.name), PCA=True, pca_array = pca_arr)
                params = SVR_read_from_file(hypers + "HP_{}.txt".format(OC.name))
